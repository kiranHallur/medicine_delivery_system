    ...
    items_container['items'].forEach((item, index)=>{
        ...
        context += `
            <tr>
                ...
                ${item['name']}
                ${
                    (item["status"]=="RETURN_REQUESTED_BY_CUSTOMER")? 
                        `<p class="required">Customer Has requested for return</p>` : ''
                }

                ${
                    (item["status"]=="RETURN_REQUEST_ACCEPTED_BY_VENDOR")? 
                        `<p class="required">Vendor has accepted customers request for return</p>` : ''
                }
                <div>
                ...
                ${
                    (!item['order_item_id'])?
                    `<button type="button" onclick="remove_item(${index}, ${null})" class="btn btn-danger btn-sm">Remove</button>` : ''
                }

                ${
                    (item['is_order_items_deducted']==true && current_user_role_id == app_constants["CUSTOMER_ROLE_ID"])?
                    `<a href="${api_url}order/item-return/${item['order_item_id']}" target="_blank" class="btn btn-warning btn-sm">Return Item</a>` : ''
                }

                ${
                    (item['is_order_items_deducted']==false && item['order_item_id'])?
                    `<button type="button" onclick="remove_item(${index}, ${item['order_item_id']})" class="btn btn-danger btn-sm">Remove</button>` : ''
                }
        ${
        ...
        ${
            (current_user_role_id == app_constants["RETAILER_ROLE_ID"] && item['is_order_items_deducted'] == false)?
                `<button type="button" onclick="deduct_item(${index}, ${item['order_item_id'] || null})" class="btn btn-success btn-sm">Deduct Item</button>`
            : ''
        }

        ${
            (current_user_role_id == app_constants["RETAILER_ROLE_ID"] && item['is_order_items_deducted'] == true)?
